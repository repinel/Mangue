...
import com.cowlark.eventbus.Event.Type;
...
public class SimpleEventBus extends EventBus {
  ...
  @Override
  public synchronized <H extends EventHandler> HandlerRegistration addHandler(Type<H> type,
      H handler) {
    if (type == null) {
  ...
  @Override
  public synchronized <H extends EventHandler> HandlerRegistration addHandlerToSource(
   ...
   */
  synchronized <H extends EventHandler> void doRemove(
      com.cowlark.eventbus.Event.Type<H> type, Object source,
      H handler) {
    if (firingDepth > 0) {
   ...
   */
  @Deprecated
  int getHandlerCount(Event.Type<?> eventKey) {
   ...
   */
  @Deprecated
  boolean isEventHandled(Event.Type<?> eventKey) {
  ...
  private <H extends EventHandler> void doAddNow(Event.Type<H> type,
      Object source, H handler) {
    List<H> l = ensureHandlerList(type, source);
    l.add(handler);
  }

  private synchronized <H extends EventHandler> void doFire(Event<H> event, Object source) {
    try {
      firingDepth++;
  ...
  private <H extends EventHandler> void enqueueAdd(final Event.Type<H> type,
